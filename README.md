...
- `Alias`: (String) Add an email alias to the INMA account in Exchange and set this value accordingly if you would like all messages to the email alias (must be in the `To` field) to be handled by this profile **Note: The INMAC account must hidden from the Global Address List in Exchange for this to work!**
